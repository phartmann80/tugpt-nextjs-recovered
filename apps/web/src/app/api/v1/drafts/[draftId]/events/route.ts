// Stage 5A: GET /api/v1/drafts/:draftId/events — List all review events for a draft
...
export async function GET(
  ...
  try {
    ...
    const events = await draftService.listReviewEvents(activeTenant.organizationId, draftId);

    defaultLogger.info('Draft review events retrieved', {
      requestId,
    ...
    defaultLogger.error('Draft review events failed', err as Error, { requestId });
