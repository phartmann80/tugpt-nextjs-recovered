...
| AI providers | OpenAI, Langdock, Mastra (via adapter pattern) |
...
| `LANGDOCK_API_CODE` | Langdock API key | **Secret** (server only) |
...
- **Structured logging**: `@tugpt/observability` provides a `Logger` generating structured JSON records with automatic secret redaction (Bearer tokens, API keys, Supabase keys) in context values.
